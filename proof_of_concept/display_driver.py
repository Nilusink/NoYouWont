...
class DisplayDriver:
    ...
    def draw_circle(
        ...
        draw_circle(buf, self._buffer_w, self._buffer_h, cx, cy, radius, thickness, color)
    ...
    def draw_rect(
        ...
        w = self._width if direct else self._buffer_w
        h = self._height if direct else self._buffer_h
        draw_rect(buf, w, h, x, y, width, height, color)
    ...
    def draw_char(
        ...
        draw_char(buf, self.width if direct else self.buffer_width, x, y, character, color)
    ...
    def draw_text(
        ...
        if background:
            self.draw_rect(
                x - background_margin, y - background_margin,
                len(text) * 8 + 2 * background_margin, 8 + 2 * background_margin,
                background_color,
                to_buffer=to_buffer,
                direct=direct
            )
        ...
        for i in range(len(text)):
            draw_char(buf, self.width if direct else self.buffer_width, x + i*8, y, ord(text[i]), color)
    ...
    def draw_digit_big(
            ...
            color: int,
            to_buffer: np.ndarray = None,
            font_size: tp.Literal[48, 96] = 48
    ) -> None:
        buf = to_buffer if to_buffer is not None else self._buffer16

        if font_size == 48:
            draw_char_48(buf, self.buffer_width, x, y, digit, color)

        elif font_size == 96:
            draw_char_96(buf, self.buffer_width, x, y, digit, color)
    ...
    def draw_digits_big(
        ...
        if center_text:
            x = round(x - (len(digits)*font_size//2) / 2) - font_size//4
            y -= font_size//2

        if background:
            self.draw_rect(
                x - background_margin, y - background_margin,
                len(digits) * (font_size//2) + 2 * background_margin + font_size//2,
                font_size + 2 * background_margin,
                background_color,
                to_buffer=to_buffer,
                direct=direct
            )
        ...
        for i in range(len(digits)):
            if font_size == 48:
                draw_char_48(
                    buf,
                    self.width if direct else self.buffer_width,
                    x + i*font_size//2,
                    y,
                    ord(digits[i]),
                    color
